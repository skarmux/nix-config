...
echo "EFI_PART"
EFI_PART=$(gum choose "/dev/nvme0n1p1" "/dev/nvme1n1p1")

echo "LUKS_PART"
LUKS_PART=$(gum choose "/dev/nvme0n1p2" "/dev/nvme1n1p2")
...
mkfs.btrfs -f -L "$FSROOT" "/dev/partitions/$FSROOT"
...
umount /mnt
mount -v -o subvol=root "/dev/partitions/$FSROOT" /mnt
mkdir -p /mnt/{etc/ssh,etc/nixos,var/log}

mkdir /mnt/nix
mount -v -o subvol=nix "/dev/partitions/$FSROOT" /mnt/nix

mkdir /mnt/persist
mount -v -o subvol=persist "/dev/partitions/$FSROOT" /mnt/persist
mkdir -p /mnt/persist/{etc/ssh,etc/nixos,var/log}

mount -v -o bind /mnt/persist/etc/ssh /mnt/etc/ssh
mount -v -o bind /mnt/persist/etc/nixos /mnt/etc/nixos
mount -v -o bind /mnt/persist/var/log /mnt/var/log
